...
$.ajax({
  ...
  complete: function(response) {
    alert('success');
    // $('.vote').find('.cancel-vote-' + problemId).attr("data-vote-id", response.id)
    ...
    // $('.upvote-' + response.problem_id).closest('.vote-container').replaceWith("<span class='cancel-vote' ><%= link_to 'Upvote', 'javascript:void(0)', data: { user_id: " + response.user_id ", problem_id: " + response.problem_id + " }, class: 'upvote-#{" + response.problem_id + "}' %></span>");
  },
...
$.ajax({
  ...
  success: function(response) {
    alert('success');
    // $('.cancel-vote-' + response.problem_id).closest('.vote-container').append('<a href="http://localhost:3000">hello</a>');
    // <a data-problem-id="1" data-vote-id="96" class="cancel-vote-1" href="javascript:void(0)">Cancel vote</a>
